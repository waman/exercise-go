...
func main(){
	const(
		...
		dx = float64(xmax-xmin)/width  // x 方向の次の画素までの距離
		dy = float64(ymax-ymin)/height // y 方向の次の画素までの距離
	)
...
// 平均を計算しやすくするため、返り値を Gray にしています。
// 200回の繰り返しの後で半径2の円の外に出ない場合の黒色も
// Gray{0} にしています。
